...
export default class CustomInterestSubscriptionList {
  ...
  load() {
    ZeeguuRequests.get(
      ...
      {},
      this._loadSubscriptions.bind(this)
    );
  }
  ...
  _addSubscription(search) {
    if (this.searchList.has(search.id)) return;
  ...
  follow(search_terms) {
    ...
    this._loading();
    let callback = (data => this._onSearchFollowed(search_terms, data)).bind(
      this
    );
    ZeeguuRequests.get(
      SUBSCRIBE_SEARCH_ENDPOINT + "/" + search_terms,
      {},
      callback
    );
  }
  ...
  _onSearchFollowed(search_terms, reply) {
    if (reply != null) {
      ...
      logger.push(
        "Could not follow '" + search_terms + "'. Server reply: \n" + reply
      );
    }
  }
  ...
  _onSearchUnfollowed(search, reply) {
    if (reply === "OK") {
      ...
      Notifier.notify(
        "Network Error - Could not unfollow " + search.search + "."
      );
      logger.push(
        "Could not unfollow '" + search.search + "'. Server reply: \n" + reply
      );
    }
  }
   ...
   */
  _remove(search) {
    if (!this.searchList.delete(search.id)) {
      console.log("Error: search not in search list.");
    }
